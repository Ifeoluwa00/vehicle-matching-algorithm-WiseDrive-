...
const convertCsvToJson_1 = require("./convertCsvToJson");
function searchFn(type, model = "", date = "") {
    // convert csv file to json
    let vehicleTypes = (0, convertCsvToJson_1.convertCsvToJson)("vehicle_types.csv");
    let parsedDate;
    ...
    if (date) {
        parsedDate = Number(date
            ...
            .join("")); //convert date to number, if string
        //validate month
        ...
        vehicleTypes = vehicleTypes.filter((vehicle) => parsedDate >= +vehicle.monthOfConstrFrom &&
            parsedDate <= +vehicle.monthOfConstrTo);
    }
    let match = [];
    if (model) {
        match = vehicleTypes.filter((vehicleObj) => {
            return (vehicleObj.typeName.replace(/\s+/g, "").toLowerCase() ===
    ...
    const bmwModels = (0, convertCsvToJson_1.convertCsvToJson)("vehicles_bmw.csv").map((vehicle) => ({
        model: vehicle.model,
        id: vehicle.id,
    ...
    match = match.filter((vehicle) => {
        return bmwModels.some((bmwmodel) => {
            ...
            let bmwmodelDate = Number(bmwmodel
                .regDate.split(" ")[0]
                .split("-")
